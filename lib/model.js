...
Model.createMethod = function (obj) {
    ...
    return function () {
        ...
        async.series([function (next) {
            ...
            var somebody = JSON.parse(JSON.stringify(body));
            somebody['api_key'] = that._pagarme.getApiKey();            

            var contentType = reqMethod == 'POST' ? 'application/json' : 'application/x-www-form-urlencoded';
            var contentSource = reqMethod == 'POST' ? JSON.stringify(somebody) : querystring.stringify(somebody);
            ...
            var requestOptions = {
                ...
                headers: {
                    ...
                    'Content-Length': contentSource.length
                }
            };
